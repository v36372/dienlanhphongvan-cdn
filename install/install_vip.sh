...
install_libvips_from_source() {
  ...
  cd ..
  rm -rf vips-8.5.6
  rm -f  vips-8.5.6.tar.gz
  ldconfig
  echo "Installed libvips $(PKG_CONFIG_PATH=$PKG_CONFIG_PATH:/usr/local/lib/pkgconfig:/usr/lib/pkgconfig:/usr/lib pkg-config --modversion vips)"
}
...
if [ -f /etc/debian_version ]; then
  ...
  case $RELEASE in
    ...
    "Fedora"*)
      ...
      echo "Installing libvips dependencies via yum"
      yum groupinstall -y "Development Tools"
      yum install -y gcc-c++ gtk-doc libxml2-devel libjpeg-turbo-devel libpng-devel libtiff-devel libexif-devel lcms-devel gobject-introspection-devel libwebp-devel curl
      install_libvips_from_source "--prefix=/usr"
      ;;
    *)
      # Unsupported RHEL-based OS
  ...
  case $RELEASE in
    "Amazon Linux AMI release 2015.03"|"Amazon Linux AMI release 2015.09")
      ...
      echo "Installing libvips dependencies via yum"
      yum groupinstall -y "Development Tools"
      yum install -y gtk-doc libxml2-devel libjpeg-turbo-devel libpng-devel libtiff-devel libexif-devel libgsf-devel lcms2-devel gobject-introspection-devel libwebp-devel curl
      install_libvips_from_source "--prefix=/usr"
      ;;
    *)
  ...
  case $RELEASE in
    *"13.2"*)
    ...
    zypper --gpg-auto-import-keys install -y --type pattern devel_basis
    zypper --gpg-auto-import-keys install -y tar curl gtk-doc libxml2-devel libjpeg-turbo libjpeg8-devel libpng16-devel libtiff-devel libexif-devel liblcms2-devel gobject-introspection-devel libwebp-devel
    install_libvips_from_source
    ;;
  esac
elif [ -f /etc/SuSE-brand ]; then
